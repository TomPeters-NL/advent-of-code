...
$input = file('./input/6', FILE_IGNORE_NEW_LINES);
...
function findGuardTurns(array $obstacles, int $x, int $y, string $direction, array $mapDimensions, array &$turns): void
{
    ...
    $obstacle = match ($direction) {
        'north' => getNearestObstacle('max', array_filter($obstacles['column'][$x] ?? [], fn ($obstacle) => $obstacle < $y)),
        'south' => getNearestObstacle('min', array_filter($obstacles['column'][$x] ?? [], fn ($obstacle) => $obstacle > $y)),
        'east' => getNearestObstacle('min', array_filter($obstacles['row'][$y] ?? [], fn ($obstacle) => $obstacle > $x)),
        'west' => getNearestObstacle('max', array_filter($obstacles['row'][$y] ?? [], fn ($obstacle) => $obstacle < $x)),
    };
    ...
    findGuardTurns($obstacles, $turnX, $turnY, $newDirection, $mapDimensions, $turns);
}
...
function detectPotentialLoops(array $obstacles, array $path, array $mapDimensions): int
{
    ...
    foreach ($potentialObstacles as [$x, $y]) {
        ...
        try {
            $turns = [];
            findGuardTurns($improvisedObstacles, $startX, $startY, 'north', $mapDimensions, $turns);
...
function partOne(array $input): int
{
    ...
    list($guardPosition, $obstacles) = analyzeMap($input);

    $turns = [$guardPosition];

    try {
        findGuardTurns($obstacles, $guardPosition['x'], $guardPosition['y'], $guardPosition['direction'], $mapDimensions, $turns);
    } catch (UnexpectedValueException) {
    }

    $path = deduceGuardPath($turns, $mapDimensions);
...
function partTwo(array $input): int
{
    ...
    return detectPotentialLoops($obstacles, $path, $mapDimensions);
}
